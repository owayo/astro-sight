...
#[test]
fn symbols_dir_reports_generated_skips_without_breaking_streaming_formats() {
    let dir = tempfile::TempDir::new().unwrap();
    std::fs::write(
        dir.path().join("generated.rb"),
        "# @generated by a tool\ndef hidden_symbol; end\n",
    )
    .unwrap();
    std::fs::write(dir.path().join("real.rb"), "def visible_symbol; end\n").unwrap();

    let output = cargo_bin()
        .args(["symbols", "--dir"])
        .arg(dir.path())
        .output()
        .expect("run symbols dir");
    assert!(output.status.success());
    let records: Vec<serde_json::Value> = String::from_utf8(output.stdout)
        ...
        .collect();
    assert_eq!(records.len(), 2, "one file plus one control record");
    assert_eq!(records[0]["path"], "real.rb");
    assert_eq!(records[1]["skipped"]["generated"], 1);
    assert_eq!(records[1]["skipped"]["paths"][0], "generated.rb");

    let toon = cargo_bin()
        .args(["--format", "toon", "symbols", "--dir"])
        .arg(dir.path())
        .output()
        .expect("run TOON symbols dir");
    assert!(toon.status.success());
    let toon = String::from_utf8(toon.stdout).unwrap();
    assert!(
        toon.starts_with("[2]:"),
        "TOON header must count the control record: {toon}"
    );
    assert!(
        toon.contains("skipped:"),
        "TOON must report skipped files: {toon}"
    );
}

#[test]
fn symbols_dir_include_generated_config_and_explicit_glob_override_filtering() {
    let dir = tempfile::TempDir::new().unwrap();
    std::fs::write(
        dir.path().join("generated.rb"),
        "# @generated by a tool\ndef hidden_symbol; end\n",
    ...
    let included = cargo_bin()
        .args(["symbols", "--include-generated", "--dir"])
        .arg(dir.path())
        .output()
        .expect("run include-generated symbols");
    assert!(included.status.success());
    let records: Vec<serde_json::Value> = String::from_utf8(included.stdout)
        .unwrap()
        .lines()
        .map(|line| serde_json::from_str(line).unwrap())
        .collect();
    assert_eq!(records.len(), 1);
    assert_eq!(records[0]["path"], "generated.rb");
    assert!(records[0].get("skipped").is_none());

    let explicit = cargo_bin()
        .args(["symbols", "--dir"])
        .arg(dir.path())
        .args(["--glob", "**/generated.rb"])
        .output()
        .expect("run explicit-glob symbols");
    assert!(explicit.status.success());
    let record: serde_json::Value = serde_json::from_slice(&explicit.stdout).unwrap();
    assert_eq!(record["path"], "generated.rb");

    let config = dir.path().join("config.toml");
    std::fs::write(&config, "skip_generated = false\n").unwrap();
    let configured = cargo_bin_with_explicit_config()
        .arg("--config")
        .arg(&config)
        .args(["symbols", "--dir"])
        .arg(dir.path())
        .output()
        .expect("run configured symbols");
    assert!(
        configured.status.success(),
        "configured symbols failed: stdout={} stderr={}",
        String::from_utf8_lossy(&configured.stdout),
        String::from_utf8_lossy(&configured.stderr)
    );
    let record: serde_json::Value = serde_json::from_slice(&configured.stdout).unwrap();
    assert_eq!(record["path"], "generated.rb");
}

#[test]
fn symbols_dir_all_generated_still_emits_skip_control_record() {
    let dir = tempfile::TempDir::new().unwrap();
    std::fs::write(
        dir.path().join("generated.rb"),
    ...
    let output = cargo_bin()
        ...
        .arg(dir.path())
        .output()
        .expect("run all-generated symbols");
    assert!(output.status.success());
    let record: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(record["skipped"]["generated"], 1);
    assert_eq!(record["skipped"]["paths"][0], "generated.rb");
}
