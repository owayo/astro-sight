...
fn context_git_diff_non_ascii_deleted_file_does_not_pollute_ascii_file() {
    ...
    .unwrap();
    std::fs::remove_file(root.join("日本語削除.ts")).unwrap();
...
fn dead_code_php_static_late_binding_stays_ambiguous() {
    ...
    .unwrap();
    std::fs::write(root.join("Consumer.php"), "<?php\n(new Child())->go();\n").unwrap();
    ...
    assert!(
        !names.contains(&"Child.helper") && !names.contains(&"Base.helper"),
...
fn dead_code_php_trait_shadowed_by_own_concrete_method_reports_dead_trait() {
    ...
    .unwrap();
    std::fs::write(
        root.join("RepoB2.php"),
        "<?php\nclass RepoB2 { use QueryB2; }\n",
    )
    .unwrap();
    std::fs::write(
        root.join("CallerWork.php"),
        "<?php\nRepo::work();\nRepoB::work();\nRepo2::work2();\nRepoB2::work2();\n",
    )
    .unwrap();
...
fn dead_code_diff_hidden_candidate_member_liveness_sees_hidden_refs() {
    ...
    assert!(
        !names.contains(&"AlphaSvc.runJob") && !names.contains(&"BetaSvc.runJob"),
...
fn review_dead_scope_touched_symbols_filters_class_members() {
    ...
    let dead_names = |args: &[&str]| -> Vec<String> {
        let output = cargo_bin().args(args).output().expect("failed to run");
        assert!(
            output.status.success(),
            "stderr: {}",
            String::from_utf8_lossy(&output.stderr)
