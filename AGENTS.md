...
- **BLAKE3** ベースのファイルキャッシュ（単一ファイル `ast` / `symbols` は内容ハッシュに canonical path と astro-sight バージョンを混ぜ、同一内容の別ファイルや別言語で `path` / `lang` が混線せず、バージョン更新時は解析ロジック/出力スキーマの変更に追従して旧キャッシュが自動失効する＝内容不変でも結果が変わるケースで stale を防ぐ）
...
- `src/cache/store.rs` - BLAKE3 ベースのキャッシュ保存（~/.cache/astro-sight/、CLI の `ast` / `symbols` は呼び出し側 `cache_hash_for_path` で path + astro-sight バージョン込みハッシュを渡し、バージョン更新時に旧キャッシュを自動失効させる）
