...
- **循環的複雑度** — symbols 出力に `cx`（cyclomatic complexity）を付加（関数/メソッドのみ、ベース1 + 分岐ノード数、ネスト関数/クロージャは除外）、言語別分岐ノード定義（Rust/JS/TS/Python/Go/Java/Kotlin/Ruby/PHP/C#等）。C/C++ は関数名を `function_declarator` 配下でキャプチャするため、シンボル抽出時に定義ノード (`function_definition`) まで親を 1 段繰り上げて range と複雑度を本体全体から算出する（繰り上げないと range が宣言子行に潰れ複雑度が常に 1 になり、impact が関数本体のみの変更を取りこぼす）
