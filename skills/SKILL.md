...
- **`--format json|toon|auto`** (global) switches the output format; the default is `json`, and `format` in `~/.config/astro-sight/config.toml` sets a different default (CLI `--format` wins). TOON v3 uses [`toon-format` 0.5](https://github.com/toon-format/toon-rust) with default features disabled. It represents data with indentation and tables; optional DTO columns may be filled with null/default values when that shortens the output. `--pretty` is JSON-only and ignored for TOON. CLI TOON documents never have a trailing newline, including batch output and TOON selected by `auto`; JSON/NDJSON retains its newline termination.
