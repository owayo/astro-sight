...
- **Generation GC**: the version is part of the cache key, so every release invalidates all entries. Invalidated entries are not deleted by themselves, and dead data used to pile up with every update (measured on a development machine: 176MB, almost all of it unreachable). Now each generation has its own directory, and the first run of a new generation deletes the old ones. Only old version directories (`v26.8.111`) and the flat 2-digit hex shards from before the generations were split (`00`–`ff`) are deleted. **Nothing with any other name is deleted** (it is under the user's `~/.cache`, so anything astro-sight cannot be sure it created is left alone). Deletion failures are ignored (analysis should keep working even when the cleanup fails)
